...
def validate_json(labels,variable_size):
    ...
    max_sequence_id=max([label['sequence_id'] for label in labels]) if variable_size else max_seq_number
    needed_identifiers = {(i,j) for i in range(min_seq_number, max_sequence_id + 1) for j in range(1, frames_per_sequence + 1)}
    missing_identifiers = needed_identifiers - set(identifiers)

    if len(missing_identifiers) > 0:
        raise ValueError('Error. Your submission needs to predict the additional following sequence_ids and frames: {}'.format(missing_identifiers))
